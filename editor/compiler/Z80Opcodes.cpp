...
void AND_A_A::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA7);
}
...
void AND_A_B::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA0);
}
...
void AND_A_C::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA1);
}
...
void AND_A_D::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA2);
}
...
void AND_A_E::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA3);
}
...
void AND_A_H::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA4);
}
...
void AND_A_L::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA5);
}
...
void AND_A_n::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xE6);
...
void AND_A_mHL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xA6);
}
...
void AND_A_mIXn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xDD);
...
void AND_A_mIYn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xFD);
...
void AND_A_IXH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xA4);
...
void AND_A_IYH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xA4);
...
void AND_A_IXL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xA5);
...
void AND_A_IYL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xA5);
...
void CP_A_A::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xBF);
}
...
void CP_A_B::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB8);
}
...
void CP_A_C::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB9);
}
...
void CP_A_D::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xBA);
}
...
void CP_A_E::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xBB);
}
...
void CP_A_H::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xBC);
}
...
void CP_A_L::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xBD);
}
...
void CP_A_n::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xFE);
...
void CP_A_mHL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xBE);
}
...
void CP_A_mIXn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xDD);
...
void CP_A_mIYn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xFD);
...
void CP_A_IXH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xBC);
...
void CP_A_IYH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xBC);
...
void CP_A_IXL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xBD);
...
void CP_A_IYL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xBD);
...
void OR_A_A::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB7);
}
...
void OR_A_B::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB0);
}
...
void OR_A_C::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB1);
}
...
void OR_A_D::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB2);
}
...
void OR_A_E::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB3);
}
...
void OR_A_H::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB4);
}
...
void OR_A_L::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xB5);
}
...
void OR_A_n::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xF6);
...
void OR_A_mHL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xB6);
}
...
void OR_A_mIXn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xDD);
...
void OR_A_mIYn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xFD);
...
void OR_A_IXH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xB4);
...
void OR_A_IYH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xB4);
...
void OR_A_IXL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xB5);
...
void OR_A_IYL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xB5);
...
void SUB_A_A::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x97);
}
...
void SUB_A_B::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x90);
}
...
void SUB_A_C::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x91);
}
...
void SUB_A_D::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x92);
}
...
void SUB_A_E::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x93);
}
...
void SUB_A_H::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x94);
}
...
void SUB_A_L::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0x95);
}
...
void SUB_A_mHL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0x96);
}
...
void SUB_A_IXH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0x94);
...
void SUB_A_IYH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0x94);
...
void SUB_A_IXL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0x95);
...
void SUB_A_IYL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0x95);
...
void SUB_A_mIXn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xDD);
...
void SUB_A_mIYn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xFD);
...
void XOR_A_B::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA8);
}
...
void XOR_A_C::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xA9);
}
...
void XOR_A_D::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xAA);
}
...
void XOR_A_E::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xAB);
}
...
void XOR_A_H::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xAC);
}
...
void XOR_A_L::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 4, 4);
    binary->emitByte(token().file, token().line, 0xAD);
}
...
void XOR_A_n::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xEE);
...
void XOR_A_mHL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 7, 7);
    binary->emitByte(token().file, token().line, 0xAE);
}
...
void XOR_A_mIXn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xDD);
...
void XOR_A_mIYn::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    ExprEvalContext context1(program, reporter, binary->endAddress(), this);
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 19, 19);
    binary->emitByte(token().file, token().line, 0xFD);
...
void XOR_A_IXH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xAC);
...
void XOR_A_IYH::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xAC);
...
void XOR_A_IXL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xDD);
    binary->emitByte(token().file, token().line, 0xAD);
...
void XOR_A_IYL::emitBinary(Program* program, ProgramBinary* binary, IErrorReporter* reporter) const
{
    binary->debugInfo()->setTStatesForLocation(token().file, token().line, 8, 8);
    binary->emitByte(token().file, token().line, 0xFD);
    binary->emitByte(token().file, token().line, 0xAD);
